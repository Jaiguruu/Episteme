...
bindings, and rewrites relationship targets in place -- it never parses, never reads
the filesystem, and never calls a language. That is what keeps it on the semantic side
of the tier boundary, and what lets one resolver cover every extractable language with
no per-language branch.

Section 4.2 governs everything here: **an explicit unresolved edge is preferred over an
incorrect confident edge.** Every rung that cannot reach certainty returns ``UNRESOLVED``
or ``AMBIGUOUS`` rather than a best guess, and section 13 AC5 asserts the property
...
Spec references (section numbers refer to SPEC.md):
    ...
    D24         -- the precedence ladder, in ``ladder.py``
    D26         -- an ambiguous edge lists its candidates, bounded
    D28         -- an ambiguous edge keeps its placeholder target
    D29         -- unresolved edges are reported, never failed
    D30         -- this package is a sibling of ``offline``, not part of it
"""
...
from ..core.contracts import Relationship, SemanticIR, Symbol
from ..core.enums import RelationshipType, ResolutionStatus, SymbolType
...
class _ModelIndex:
    ...
    The naive form of every rung rescans ``ir.symbols`` or ``ir.relationships``. On a
    large repository that turns resolution from milliseconds into minutes, so the
    lookups are built once here and the rungs only read them.
    """

    #: symbol id -> Symbol
    symbols: dict[str, Symbol]
    #: qualified_name -> symbols with that name. A list, because a name is not a key:
    #: two modules may declare the same qualified name.
    ...
    #: qualified name of a containing class -> its members
    members_of: dict[str, list[Symbol]] = field(default_factory=dict)
    #: symbol id -> the qualified name of its enclosing class, if any
    ...
    #: every qualified name in the model, for the attribute-chain rungs
    names: frozenset[str] = frozenset()

    @classmethod
    def build(cls, ir: SemanticIR) -> "_ModelIndex":
        """Build every lookup in a bounded number of passes over the model."""
        index = cls(symbols={symbol.id: symbol for symbol in ir.symbols})
        ...
        # Class membership. A member's qualified name is ``pkg.mod:Class.member``, so
        # the owner is the text before the last dot *within the part after the colon*.
        # This is the one structural convention the extractor guarantees, and it is
        # language-neutral: it describes nesting, not grammar.
        for symbol in ir.symbols:
        ...
        # Imports, for rung S4. An IMPORTS edge is emitted from a file's module symbol,
        # so every import in a file is attributed to that file id.
...
def _enclosing_class_name(symbol: Symbol) -> str | None:
    ...
    A module symbol, or a top-level function, has no owner and returns ``None``. The
    rule is purely about the shape of the qualified name, so it says nothing about any
...
class ResolutionReport:
    ...
    resolver*, not an attribute of an edge, so persisting it into the model would tie
    the model schema to the algorithm that produced it -- and a better resolver would
    then need a schema migration.
    """

    #: rung -> how many edges it answered
    counts: dict[ResolutionRung, int] = field(default_factory=dict)
    #: edges whose final status is RESOLVED_EXACT or RESOLVED_HEURISTIC
    resolved: int = 0
    #: edges whose final status is AMBIGUOUS
    ambiguous: int = 0
    #: edges whose final status is UNRESOLVED
    unresolved: int = 0
    #: edges whose target actually changed, for a reuse/re-resolution diff
    retargeted: int = 0
    ...
    def to_dict(self) -> dict[str, object]:
        """Serialise with ``by_rung`` in ladder order, so output is stable."""
        return {
            "by_rung": {
...
class Resolver:
    ...
    def _resolve_one(
        ...
        # Strip any call syntax the raw text may still carry: the extractor captures
        # the callee expression, and a trailing ``()`` is not part of a name.
        lookup = relationship.target_name.split("(", 1)[0].strip()
        if not lookup:
            report.record(ResolutionRung.NONE)
            return
    ...
    def _walk_ladder(
        ...
        """Dispatch by syntactic shape, then let the ladder order decide.

        The dispatch order below is by *shape* -- receiver forms first, then bare names
        -- while the rung numbering in ``ladder.py`` is by *evidence strength*. The two
        orders agree on every outcome because the cases are disjoint: a name beginning
        with an instance receiver is never also a bare local name.
        """
        head = name.split(".", 1)[0]

        # S2/S3: an instance receiver. ``self.validate`` is a member of the class the
        # caller lives in; ``self.payment_service.process`` needs the binding.
        ...
        # S7/S8: a dotted name whose head may still be placeable.
        if "." in name:
            rung, qualified_names = resolve_qualified_name(name, self.index.names)
            if rung is not ResolutionRung.NONE:
                resolved = [
                    symbol
                    for qualified_name in qualified_names
                    for symbol in self.index.by_qualified_name.get(qualified_name, [])
                ]
                if resolved:
    ...
    def _resolve_receiver_chain(
        ...
        """Handle ``self.m``, ``self.x`` and ``self.x.m``.

        ``self.m()`` is a member of the caller's own class (S2). ``self.x.m()`` needs
        the declared type of ``x``, which is a ``Binding`` (S3) -- exactly the path
        bindings were persisted for.
        """
        parts = name.split(".")

        # ``self.member`` -- the owner class is known from the enclosing symbol, so
        # this is a closed-set lookup and certain.
        if len(parts) == 2:
            ...
            # Not a declared member. The class may inherit it from a base we can see,
            # so try that before giving up rather than guessing.
        ...
        if len(parts) >= 3:
            ...
            if len(type_symbols) > 1:
                # The same type name in two modules: ambiguous, and we say so rather
                # than picking one.
                candidates = [
                    symbol
    ...
    def _same_file(self, source: Symbol, name: str) -> Symbol | None:
        ...
        Returns ``None`` when several match, because "the one in this file" is then not
        a unique answer and the weaker rungs should decide instead.
        """
        matches = [
            symbol
    ...
    def _imported(self, source: Symbol, name: str) -> list[Symbol]:
        ...
        ``service.checkout`` (an import chain whose head names a module). The match is
        on the *simple* name against the imported module's symbols, which is what an
        author means by writing an imported name in a call.
        """
        ...
        for module_name in imported_modules:
            for symbol in self.index.by_file.get(
                ...
                if tail:
                    ...
                    # imported module, or of a class the module declares.
                    if symbol.name == tail and symbol.qualified_name.startswith(
                        f"{module_name}:"
                    ):
                        matches.append(symbol)
                elif symbol.name == head or symbol.qualified_name.endswith(f":{head}"):
                    matches.append(symbol)
    ...
    def _unique_in_model(self, name: str) -> tuple[ResolutionRung, list[Symbol]]:
        """A name that appears exactly once in the whole model resolves (S5).

        Several matches are reported as ambiguous with their candidates rather than
        resolved by an arbitrary pick (section 13 AC3, D26).
        """
    ...
    def _bound_type_of(self, source: Symbol, receiver: str) -> str | None:
        ...
        Two scopes can apply and the narrower wins: a binding in the caller's own body
        (a local), then one in the class constructor (an instance attribute, which is
        where ``self.x = ...`` is recorded). A local wins because it is closer to the
        call site.

        An unannotated alias is followed one hop. ``self.repository = repository``
        records the type as the literal text ``repository``, which names another bound
        thing rather than a class; when that name is itself bound in the same
        constructor, its type is the answer. This is how the common
        constructor-injection shape resolves, and it is language-neutral -- it says
        "a type name that is also a bound name is an alias", not anything about Python.
        """
        owners = [source.id]
        ...
        # repository: PaymentRepository)`` produces both a PARAMETER binding carrying
        # the real type and an INSTANCE binding for ``self.repository = repository``
        # carrying the literal text ``repository``. Taking the first match would pick
        # whichever the extractor happened to emit first, so choose by declared quality
        # instead.
        candidates: list[str] = []
    ...
    def _best_type_name(candidates: list[str], receiver: str) -> str | None:
        ...
        ``repository: repository`` -- and carries no type information. It is therefore
        the last resort, and a declared type in the same scope wins.
        """
    ...
    def _binding_named(self, name: str, owner_ids: list[str]) -> str | None:
        ...
        for owner_id in owner_ids:
            for binding in self.ir.bindings:
                if (
                    binding.enclosing_symbol_id == owner_id
                    and binding.bound_name == name
                ):
                    return binding.type_name
        return None

    def _constructor_of(self, source: Symbol) -> Symbol | None:
        """The constructor of the caller's class, if the model names one."""
        owner = self.index.owner_class.get(source.id)
        if owner is None:
            return None
        for symbol in self.index.members_of.get(owner, []):
            if symbol.name in _CONSTRUCTOR_NAMES:
                return symbol
        return None
    ...
    def _class_named(self, type_name: str) -> list[Symbol]:
        ...
        The binding records the type as written, which may or may not be qualified, so
        both forms are accepted. More than one match is returned rather than broken by
        an arbitrary pick -- the caller reports it as ambiguous.
        """
        simple = type_name.rsplit(".", 1)[-1]
        matches = [
            symbol
            for symbol in self.index.by_name.get(simple, [])
            if symbol.symbol_type in _TYPE_SYMBOL_TYPES
        ]
        if matches:
            return matches
    ...
    def _member_of(self, owner_qualified_name: str, member: str) -> Symbol | None:
        """A member declared directly on a class, by simple name."""
    ...
    def _inherited_member(
        ...
        ``Base.validate`` case, and shallow enough that the edge it produces is still
        traceable to a declared relationship rather than to a search.
        """
        source_symbol = next(
            iter(self.index.by_qualified_name.get(owner_qualified_name, [])), None
        )
        ...
        for relationship in self.ir.relationships:
            ...
            if not relationship.target_name:
                continue
            for base in self._class_named(relationship.target_name):
    ...
    def _apply(
        ...
        This is the whole correctness argument in one place: an edge either points at a
        symbol the model justifies, or it is explicitly marked as not doing so.
        """
        report.record(rung)

        if rung is ResolutionRung.NONE or not candidates:
            # D29: leave it explicitly unresolved, and keep ``target_name`` so the raw
            # text stays available for a later pass.
        ...
        if rung.status is ResolutionStatus.AMBIGUOUS:
            # D28: an ambiguous edge keeps its placeholder target, so edge counts do
            # not inflate and a caller must consult ``resolution_status`` before
            # trusting ``target_symbol_id``.
            #
            # The candidate list is capped. It exists so a reader can *act* on the
            # ambiguity, and 135 ids for a name that is simply common across many
            # generated modules is not actionable -- it is a payload. The cap keeps
            # the marker honest (the edge is still AMBIGUOUS) while the stored list
            # only claims to be a bounded set of the closest candidates.
            ordered = sorted(
        ...
        # Exact: point at the winner, chosen deterministically rather than by
    ...
    def _tally(self, report: ResolutionReport) -> None:
        """Count every edge by its final status, so the totals match the model."""
        for relationship in self.ir.relationships:
...
#: already exact from Stage 5; inventing rungs for it would be scope for no benefit.
_RESOLVABLE: Final[frozenset[RelationshipType]] = frozenset(
    {RelationshipType.CALLS, RelationshipType.IMPORTS, RelationshipType.INHERITS}
)

#: Symbol types that can be the type of a bound receiver -- the kinds that own
#: members. ``SymbolType`` is deliberately small and language-neutral, so a Rust
#: ``struct`` or a Go ``type`` arrives here as ``CLASS`` rather than as a
#: language-specific kind (section 11 AC3).
_TYPE_SYMBOL_TYPES: Final[frozenset[SymbolType]] = frozenset(
    {SymbolType.CLASS, SymbolType.INTERFACE, SymbolType.ENUM}
)

#: Constructor names across the languages this project extracts. Data rather than
#: branching, for the same reason ``INSTANCE_RECEIVERS`` is.
_CONSTRUCTOR_NAMES: Final[frozenset[str]] = frozenset({"__init__", "constructor", "new"})

#: Upper bound on the candidates recorded for an ambiguous edge (D26).
#:
#: Chosen so the list stays readable in a report and cheap to serialise, while still
#: naming enough targets to disambiguate a real case. A name common across every
#: generated module in a large repository can match hundreds of symbols, and storing
#: all of them turns a diagnostic into a payload without helping anyone.
